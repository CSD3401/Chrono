...
* 2) Player repeats the sequence using keyboard keys 1..9.
*
* Notes:
* - Assign as many lights as you want (light1..light9). Only valid refs are used.
* - For future clickable buttons: call ReceiveInput(n) where n is 1..numLights.
...
public:
    ...
    void Start() override {
        BuildLightListFromSlots();

        if (m_numLights < 2) {
            LOG_WARNING("[Sequencer] Need at least 2 assigned lights (light1..light9).");
            m_state = State::Idle;
            return;
        }
        ...
        // If left empty in inspector, fall back to a default
        if (solvedEventName.empty()) solvedEventName = "PuzzleSolved";

        SetAllLights(false);

        if (autoStart) {
            BeginNewRound(/*reseed=*/true);
        }
    }
    ...
    void Update(double deltaTime) override {
        ...
        if (m_state == State::FailedDelay) {
            m_timer -= dt;
            if (m_timer <= 0.0f) {
                BeginNewRound(/*reseed=*/false); // replay same sequence
            }
            return;
        }
        ...
        if (m_state == State::WaitingInput) {
            const int maxKeys = (m_numLights < 9) ? m_numLights : 9;
    ...
    void ReceiveInput(int input) {
        ...
        int idx = input;
        if (idx >= 1 && idx <= m_numLights) idx -= 1;
        if (idx < 0 || idx >= m_numLights) return;

        const int expected = m_sequence[m_inputIndex];
        const bool correct = (idx == expected);

        FlashFeedback(idx);

        if (!correct) {
            Unsolve(); // now a no-op (no event)
...
private:
    ...
    void BuildLightListFromSlots() {
        ...
        for (auto& s : slots) {
            if (s.IsValid()) m_lights.push_back(s);
        }

        m_numLights = static_cast<int>(m_lights.size());

        m_defaultMaterialUUID.clear();
        m_defaultMaterialUUID.resize(m_lights.size());

        for (size_t i = 0; i < m_lights.size(); ++i) {
            m_defaultMaterialUUID[i] = NE::Renderer::Query::GetMaterial(m_lights[i].GetEntity());
        }
    }
    ...
    void GenerateSequence(bool reseed) {
        ...
        std::uniform_int_distribution<int> dist(0, m_numLights - 1);
        m_sequence.clear();
        m_sequence.reserve(static_cast<size_t>(sequenceLength));

        for (int i = 0; i < sequenceLength; ++i) {
            m_sequence.push_back(dist(m_rng));
        }

        if (logSequence) {
            std::stringstream ss;
            ss << "[Sequencer] Lights=" << m_numLights << " Sequence: ";
    ...
    void SetLightState(int lightIndex, bool on) {
        if (lightIndex < 0 || lightIndex >= m_numLights) return;
        if (!m_lights[lightIndex].IsValid()) return;

        const Entity e = m_lights[lightIndex].GetEntity();
        ...
        else if (!m_defaultMaterialUUID[lightIndex].empty() &&
            m_defaultMaterialUUID[lightIndex] != "empty uuid") {
            NE::Renderer::Command::AssignMaterial(e, m_defaultMaterialUUID[lightIndex]);
        }
    }

    void SetAllLights(bool on) {
        for (int i = 0; i < m_numLights; ++i) SetLightState(i, on);
    }

    void ShowCurrentLight(bool on) {
        SetAllLights(false);
        if (m_showIndex < 0 || m_showIndex >= static_cast<int>(m_sequence.size())) return;
        SetLightState(m_sequence[m_showIndex], on);
    }

    void FlashFeedback(int lightIndex) {
        if (lightIndex < 0 || lightIndex >= m_numLights) return;

        auto handle = Coroutines::Create();
        Coroutines::AddAction(handle, [this, lightIndex]() { SetLightState(lightIndex, true); });
        Coroutines::AddWait(handle, 0.08f);
        Coroutines::AddAction(handle, [this, lightIndex]() {
            if (m_state == State::WaitingInput) SetLightState(lightIndex, false);
            });
...
private:
    ...
    std::string solvedEventName = "SequencerSolved";

    // === Runtime ===
    std::vector<GameObjectRef> m_lights{};
    std::vector<std::string> m_defaultMaterialUUID{};
    int m_numLights = 0;
